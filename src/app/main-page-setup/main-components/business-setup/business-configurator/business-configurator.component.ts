...
@Component({
  selector: 'app-business-configurator',
  templateUrl: './business-configurator.component.html',
  styleUrl: './business-configurator.component.css',

})
export class BusinessConfiguratorComponent {
  Toast = Swal.mixin({
    toast: true,
    position: 'top-right',
...
this.ShowNewBusinessField = false
  ...
  FileByApi:any;
  InsertNewBusiness(){
    if(this.BusinessForm.value.logoUrl && this.BusinessForm.value.business){
      const a:any = this.BusinessForm.value.logoUrl;
      const fileData = a[0];
      ...
      this.service.uploadFile(formData).subscribe((res) => {
        ...
        console.log("Upload",res, this.FileByApi)
      
        this.InsertBusinessWithLogo(this.FileByApi);
      
      });
    }else{
      this.Toast.fire({
        icon: 'info',
        title: 'Enter business and logo',
      }) 
    }
   
  }

  InsertBusinessWithLogo(logo:any){
    const businessObj = {
    ...
    if(businessObj.business != null && businessObj.business != "" && businessObj.business != undefined && businessObj.logoUrl != null && businessObj.logoUrl != "" && businessObj.logoUrl != undefined){
      this.service.PostBusinessName(businessObj).subscribe((res:any)=>{
        console.log("New Business",res);
        if(res.id){
  ...
  SubmitBusiness(){
    // this.busienssLabel = this.BusinessForm.value.business;
     ...
     if(this.businessObj.followupFields){
      this.listData = this.businessObj.followupFields;
     }

     this.service.GetFormlyForm(this.businessObj.businessId).subscribe((val:any)=>{
...
createdAt:this.CurrentDate,
updatedAt:this.CurrentDate,
followupFields:CompletefollowupFields,
ticketFields:this.businessObj.ticketFields,
...
////////////////////Post Formly obj/////////////////
const CompleteBusinessObj =  this.CompleteBusiness.followupFields;
...
updatedAt:this.CurrentDate,
 ...
 this.service.PostFormlyForm(FormlyFinalObj).subscribe((res:any)=>{
...
this.GetBusiness();
 ...
 ResetAllArrays(){
  ...
  DataTypes:any=[]
  ngOnInit() {
    this.DataTypes = ['Number', 'Text', 'Dropdown list'];
    this.options = ['POS', 'ATM', 'MBPP'];
  }
